...
def get_complete_forecast(http_get_fn: HttpGetFn, api_key: str, city: str) -> dict[str, Any] | None:
    ...
    print(f"Couldn't find weather data. Check '{city}' if it exists and is correctly spelled.\n")
    return None


def get_temperature(full_weather_forecast: dict[str, Any]) -> float:
    temperature = full_weather_forecast["main"]["temp"]
    return round(temperature - 273.15, 1)  # convert from Kelvin to Celsius
