...
with open("./processors.json", "r", encoding="utf-8") as file:
    processors = json.load(file)

    for processor in processors:
        loader.load_plugins(processors["plugins"])
...
def handle_payment(total: Decimal) -> None:
    payment_type = input(
        f"What payment method would you like to use? ({', '.join(payment_processor_factory.list_processors())})\n"
    )

    if payment_type in payment_processor_factory.list_processors():
        # Process the payment
        payment_processor_factory.get(payment_type)(total)
    else:
