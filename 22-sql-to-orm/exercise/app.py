...
SQL_DATABASE_URL = "sqlite:///./events.db"
...
async def get_all_events(database: Session = Depends(get_db)):
    event = database.query(Event).all()
    return event
...
async def book_ticket(ticket: TicketCreate, database: Session = Depends(get_db)):
    ...
    if not event:
        raise HTTPException(status_code=404, detail=f"Event with id {ticket.event_id} not found")
...
async def get_ticket(ticket_id: int, database: Session = Depends(get_db)):
    ...
    if not ticket:
        raise HTTPException(status_code=404, detail=f"Ticket with id {Ticket.id} not found")
    return ticket
